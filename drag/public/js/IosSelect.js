/**
 * IosSelect
 * @param {number} level 閫夋嫨鐨勫眰绾� 1 2 3 4 5 6 鏈€澶氭敮鎸�6灞�
 * @param {...Array} data [oneLevelData[, twoLevelData[, threeLevelData[, fourLevelData[, fiveLevelData[, sixLevelData]]]]]] 鍙互鐢ㄦ暟缁勶紝涔熷彲浠ョ敤鏂规硶
 * @param {Object} options
 * @param {string=} options.container 缁勪欢鎻掑叆鍒拌鍏冪礌涓� 鍙€�
 * @param {Function} options.callback 閫夋嫨瀹屾瘯鍚庣殑鍥炶皟鍑芥暟
 * @param {Function} options.fallback 閫夋嫨鍙栨秷鍚庣殑鍥炶皟鍑芥暟
 * @param {Function} options.maskCallback 鐐瑰嚮鑳屾櫙灞傚叧闂粍浠舵椂瑙﹀彂鐨勬柟娉�
 * @param {string=} options.title 閫夋嫨妗唗itle
 * @param {number=} options.itemHeight 姣忎竴椤圭殑楂樺害锛岄粯璁� 35
 * @param {number=} options.itemShowCount 缁勪欢灞曠ず鐨勯」鏁帮紝榛樿 7锛屽彲閫�3,5,7,9锛屼笉杩囦笉鏄�3,5,7,9鍒欏睍绀�7椤�
 * @param {number=} options.headerHeight 缁勪欢鏍囬鏍忛珮搴� 榛樿 44
 * @param {css=} options.cssUnit px鎴栬€卹em 榛樿鏄痯x
 * @param {string=} options.addClassName 缁勪欢棰濆绫诲悕 鐢ㄤ簬鑷畾涔夋牱寮�
 * @param {...Array=} options.relation 鏁扮粍 [oneTwoRelation, twoThreeRelation, threeFourRelation, fourFiveRelation] 榛樿鍊硷細[0, 0, 0, 0, 0, 0]
 * @param {number=} options.relation.oneTwoRelation 绗竴鍒楀拰绗簩鍒楁槸鍚﹂€氳繃parentId鍏宠仈
 * @param {number=} options.relation.twoThreeRelation 绗簩鍒楀拰绗笁鍒楁槸鍚﹂€氳繃parentId鍏宠仈
 * @param {number=} options.relation.threeFourRelation 绗笁鍒楀拰绗洓鍒楁槸鍚﹂€氳繃parentId鍏宠仈
 * @param {number=} options.relation.fourFiveRelation 绗洓鍒楀拰绗簲鍒楁槸鍚﹂€氳繃parentId鍏宠仈
 * @param {number=} options.relation.fiveSixRelation 绗簲鍒楀拰绗叚鍒楁槸鍚﹂€氳繃parentId鍏宠仈
 * @param {string=} options.oneLevelId 绗竴绾ч€変腑id
 * @param {string=} options.twoLevelId 绗簩绾ч€変腑id
 * @param {string=} options.threeLevelId 绗笁绾ч€変腑id
 * @param {string=} options.fourLevelId 绗洓绾ч€変腑id
 * @param {string=} options.fiveLevelId 绗簲绾ч€変腑id
 * @param {string=} options.sixLevelId 绗叚绾ч€変腑id
 * @param {boolean=} options.showLoading 濡傛灉浣犵殑鏁版嵁鏄紓姝ュ姞杞界殑锛屽彲浠ヤ娇鐢ㄨ鍙傛暟璁剧疆涓簍rue锛屼笅鎷夎彍鍗曚細鏈夊姞杞戒腑鐨勬晥鏋�
 * @param {boolean=} options.showAnimate 鏄惁闇€瑕佸叆鍦哄姩鐢诲拰閫€鍦哄姩鐢伙紝濡傞渶鑷畾涔夊姩鐢绘晥鏋滐紝璇蜂慨鏀筩ss
 */
(function () {
  ...
  var utils = (function () {
      ...
      me.momentum = function (current, start, time, lowerMargin, wrapperSize, deceleration) {
          ...
          if (destination < lowerMargin) {
              destination = wrapperSize ? lowerMargin - (wrapperSize / 2.5 * (speed / 8)) :
                  lowerMargin;
      ...
      me.extend(me.ease = {}, {
          ...
          elastic: {
              ...
              fn: function (k) {
                  ...
                  return (e * Math.pow(2, -10 * k) * Math.sin((k - f / 4) * (2 * Math.PI) / f) +
                      1);
              }
          }
      });
  ...
  IScrollForIosSelect.prototype = {
      ...
      _start: function (e) {
          ...
          if (this.options.preventDefault && !utils.isBadAndroid && !utils.preventDefaultException(e.target,
      ...
      _end: function (e) {
          ...
          if (this.options.momentum && duration < 300) {
              momentumX = this.hasHorizontalScroll ? utils.momentum(this.x, this.startX, duration, this.maxScrollX,
                  this.options.bounce ? this.wrapperWidth : 0, this.options.deceleration) : {
                  destination: newX,
                  duration: 0
              };
              momentumY = this.hasVerticalScroll ? utils.momentum(this.y, this.startY, duration, this.maxScrollY,
      ...
      scrollToElement: function (el, time, offsetX, offsetY, easing) {
          ...
          time = time === undefined || time === null || time === 'auto' ? Math.max(Math.abs(this.x - pos.left),
              Math.abs(this.y - pos.top)) : time;
  ...
  Layer.prototype = {
      init: function () {
          ...
          this.el.style.height = Math.max(document.documentElement.getBoundingClientRect().height, window
              .innerHeight);
  ...
  function IosSelect(level, data, options) {
      ...
      this.options.itemShowCount = [3, 5, 7, 9].indexOf(options.itemShowCount) !== -1 ? options.itemShowCount : 5;
  ...
  IosSelect.prototype = {
      init: function () {
          this.initLayer();
          this.setLevelData(1, this.options.oneLevelId, this.options.twoLevelId, this.options.threeLevelId,
              this.options.fourLevelId, this.options.fiveLevelId, this.options.sixLevelId);
      },
      initLayer: function () {
          var self = this;
          var sureText = this.options.sureText || '确认';
          var closeText = this.options.closeText || '取消';
          var headerHeightCss = this.options.headerHeight + this.options.cssUnit;
          var all_html = [
              '<header style="height: ' + headerHeightCss + '; line-height: ' + headerHeightCss +
              '" class="iosselect-header">',
                  '<a style="height: ' + headerHeightCss + '; line-height: ' + headerHeightCss +
              '" href="javascript:void(0)" class="close">' + closeText + '</a>',
                  '<a style="height: ' + headerHeightCss + '; line-height: ' + headerHeightCss +
              '" href="javascript:void(0)" class="sure">' + sureText + '</a>',
          ...
          this.iosSelectLayer = new Layer(all_html, {
              className: 'ios-select-widget-box ' + this.typeBox + (this.options.addClassName ?
                  ' ' + this.options.addClassName : '') + (this.options.showAnimate ?
                  ' fadeInUp' : ''),
          ...
          if (this.options.headerHeight && this.options.itemHeight) {
              ...
              this.coverArea1Dom.style.top = this.options.headerHeight + this.options.itemHeight * this.options
                  .coverArea1Top + this.options.cssUnit;
              this.coverArea2Dom = this.iosSelectLayer.el.querySelector('.cover-area2');
              this.coverArea2Dom.style.top = this.options.headerHeight + this.options.itemHeight * this.options
                  .coverArea2Top + this.options.cssUnit;
          }
          ...
          this.iosSelectLayer.el.querySelector('.layer').style.height = this.options.itemHeight * this.options
              .itemShowCount + this.options.headerHeight + this.options.cssUnit;

          this.oneLevelContainDom.style.height = this.options.itemHeight * this.options.itemShowCount +
          ...
          if (this.level >= 2) {
              this.twoLevelContainDom.style.height = this.options.itemHeight * this.options.itemShowCount +
                  this.options.cssUnit;
          ...
          if (this.level >= 3) {
              this.threeLevelContainDom.style.height = this.options.itemHeight * this.options.itemShowCount +
                  this.options.cssUnit;
          ...
          if (this.level >= 4) {
              this.fourLevelContainDom.style.height = this.options.itemHeight * this.options.itemShowCount +
                  this.options.cssUnit;
          ...
          if (this.level >= 5) {
              this.fiveLevelContainDom.style.height = this.options.itemHeight * this.options.itemShowCount +
                  this.options.cssUnit;
          ...
          if (this.level >= 6) {
              this.sixLevelContainDom.style.height = this.options.itemHeight * this.options.itemShowCount +
                  this.options.cssUnit;
          ...
          this.selectBtnDom.addEventListener('click', function (e) {
              self.options.callback && self.options.callback(self.selectOneObj, self.selectTwoObj,
                  self.selectThreeObj, self.selectFourObj, self.selectFiveObj, self.selectSixObj
              );
          });
      },
      ...
      setScorllEvent: function (scrollInstance, index) {
          ...
          scrollInstance.on('scrollEnd', function () {
              ...
              if (self.level > index) {
                  if ((mapKey.relation === 1 && iosSelectUtil.isArray(self.data[index])) ||
                      iosSelectUtil.isFunction(self.data[index])) {
                      self.setLevelData(index + 1, self.selectOneObj.id, self.selectTwoObj.id,
                          self.selectThreeObj.id, self.selectFourObj.id, self.selectFiveObj.id,
                          self.selectSixObj.id);
                  }
              }
          });
      ...
      setLevelData: function (index, oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId,
          sixLevelId) {
          if (iosSelectUtil.isArray(this.data[index - 1])) {
              ...
              this.renderLevel(index, oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId,
                  sixLevelId, levelData);
          } else if (iosSelectUtil.isFunction(this.data[index - 1])) {
              this.loadingShow();
              this.data[index - 1].apply(this, [oneLevelId, twoLevelId, threeLevelId, fourLevelId,
                  fiveLevelId].slice(0, index - 1).concat(function (levelData) {
                  this.loadingHide();
                  this.renderLevel(index, oneLevelId, twoLevelId, threeLevelId, fourLevelId,
                      fiveLevelId, sixLevelId, levelData);
              }.bind(this)));
          } else {
              throw new Error('data format error');
          }
      },
      renderLevel: function (index, oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId,
          sixLevelId, levelData) {
          var plast = 0;
          ...
          levelData.forEach(function (v, i, o) {
              if (v.id == curLevelId) {
                  tmpHtml += '<li style="height: ' + itemHeightStyle + '; line-height: ' +
                      itemHeightStyle + ';"' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v.value +
                      '</li>';
                  plast = i + 1;
              } else {
                  tmpHtml += '<li style="height: ' + itemHeightStyle + '; line-height: ' +
                      itemHeightStyle + ';"' + iosSelectUtil.attrToHtml(v) + '>' + v.value +
                      '</li>';
              }
          });
          tmpHtml += this.getWhiteItem();
          ...
          if (this.level > index) {
              this.setLevelData(index + 1, this.selectOneObj.id, this.selectTwoObj.id, this.selectThreeObj
                  .id, this.selectFourObj.id, this.selectFiveObj.id, this.selectSixObj.id);
          }
      },
      ...
      getWhiteItem: function () {
          var whiteItemHtml = '';
          var itemHeightStyle = this.options.itemHeight + this.options.cssUnit;
          var itemLi = '<li style="height: ' + itemHeightStyle + '; line-height: ' + itemHeightStyle +
              '"></li>';
